...
#busqueda en el ads_modificado_por_carlos

def ads_search_tool(
    query: str, max_results: int = 5
) -> list[dict]:
    """
    Search NASA ADS for scientific literature.
    ...
    Returns:
        List[dict]: Papers found in NASA ADS.
    """
    print(f"🔭 NASA ADS: buscando '{query}'")
    api_key = os.getenv("ADS_API_KEY")

    if not api_key:
        raise ValueError("ADS_API_KEY not found in environment variables.")
    ...
    params = {
        "q": query,
        "fl": "title,author,year,bibcode,doi,abstract,pub,arxiv_class",
        "rows": max_results,
        "sort": "date desc",
    }
...
ads_tool_def = {
    ...
    "function": {
        ...
        "description": (
            "Searches NASA ADS for scientific literature in astronomy "
            "and astrophysics. Returns papers, authors, abstracts, "
            "publication information, DOI, bibcode, and ADS URLs."
        ),
        "parameters": {
            "type": "object",
            "properties": {
                "query": {
                    "type": "string",
                    "description": (
                        "Scientific search query for NASA ADS."
                    ),
                },
                "max_results": {
                    "type": "integer",
...
#MODIFICADO PARA AGREGAR EL ADS:
